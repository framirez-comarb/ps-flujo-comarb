"""Cuántos eventos PS del día de ayer (2026-05-04) llegan sin `pantalla`."""
import io
import sys
...
print(f"Custom dim: customEvent:pantalla\n")

stats = defaultdict(lambda: {"con": 0, "sin": 0, "valores": defaultdict(int)})
total_con = 0
total_sin = 0
...
print(f"  Con pantalla:         {total_con} ({pct_global}%)")
print(f"  Sin pantalla:         {total_sin} ({round(100-pct_global,1)}%)")
